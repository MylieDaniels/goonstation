			...
			if (!l_arm || !r_arm || !l_leg || !r_leg)
				...
				SPAWN(2 SECONDS)
					if (H && src && (src in H.implant))
						...
						if (!l_arm)
							H.limbs.replace_with(LIMB_LEFT_ARM, pick(left_arm), null, 0)
						else if (!r_arm)
							H.limbs.replace_with(LIMB_RIGHT_ARM, pick(right_arm), null, 0)
						else if (!l_leg)
							H.limbs.replace_with(LIMB_LEFT_LEG, pick(left_leg), null, 0)
						else if (!r_leg)
							H.limbs.replace_with(LIMB_RIGHT_LEG, pick(right_leg), null, 0)
						H.update_body()
...
/obj/item/implant/artifact/ancient/ancient_bad
	...
	do_process(var/mult = 1)
		...
		if (H.get_oxygen_deprivation() > 175)
			...
			SPAWN(3 SECONDS)
				for (var/obj/item/mob_part/humanoid_part.limb in H.limbs.parts)
					if (H && src)
