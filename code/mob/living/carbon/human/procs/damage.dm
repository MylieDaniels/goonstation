...
/mob/living/carbon/human/TakeDamage(zone, brute, burn, tox, damage_type, disallow_limb_loss, var/bypass_reversal = FALSE)
	...
	if ((src.health - brute) <= -100 && brute >= 5 && !ON_COOLDOWN(src, "Death from Impact", 4 DECI SECONDS) && !(src.reagents && src.reagents.has_reagent("bodmir_draksadd"))) // stupid bullet hoses
