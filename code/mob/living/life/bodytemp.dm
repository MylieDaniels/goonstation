...
/mob/living/proc/handle_temperature_damage(body_part, exposed_temperature, exposed_intensity, mult)
	...
	switch(body_part)
		...
		if (LEGS)
			TakeDamage(LIMB_LEFT_LEG, 0, 0.6*discomfort*mult, 0, DAMAGE_BURN)
			TakeDamage(LIMB_RIGHT_LEG, 0, 0.6*discomfort*mult, 0, DAMAGE_BURN)
		if (ARMS)
			TakeDamage(LIMB_LEFT_ARM, 0, 0.4*discomfort*mult, 0, DAMAGE_BURN)
			TakeDamage(LIMB_RIGHT_ARM, 0, 0.4*discomfort*mult, 0, DAMAGE_BURN)
