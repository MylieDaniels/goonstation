		...
		if (src.canmove)
			...
			if (src.buckled && !istype(src.buckled, /obj/stool/chair))
				...
				if (!spacemove) // buh
					...
					if(ishuman(src) && !src?.client?.flying && !src.hasStatus("resting") && !src.buckled && !H.limbs.slot_filled(LIMB_LEGS_BOTH))	//do this before we move, so we can dump stuff on the old tile. Just to be mean.
